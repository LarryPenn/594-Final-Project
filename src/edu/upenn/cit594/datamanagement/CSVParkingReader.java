...
public class CSVParkingReader extends Reader implements ParkingReader  {
	...
	public Parking readParkingData() {
			...
			while (line != null) {
				...
				if (splitLine.length == 7) {
					...
					if (zipcodeCheck(potentialZipcode) && valueCheck(potentialFine)) {
						...
						//System.out.println(getZipcode(potentialZipcode)+ " " + getValue(potentialFine));
		...
		//for ( Integer key : parkingData.getData().keySet() ) {System.out.println(key + " " + parkingData.getData().get(key));}
